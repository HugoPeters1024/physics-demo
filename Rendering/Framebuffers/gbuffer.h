...
{
private:
    GLuint m_id, m_tex_normal, m_tex_pos, m_tex_albedo, m_tex_lighting, m_depth_object;
...
public:
    GBuffer() {
      ...
      glGenTextures(1, &m_tex_lighting);
      glBindTexture(GL_TEXTURE_2D, m_tex_lighting);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGBA, GL_FLOAT, nullptr);
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
      glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, m_tex_lighting, 0);
    ...
    GLuint getLightingTexture() const { return m_tex_lighting; }
