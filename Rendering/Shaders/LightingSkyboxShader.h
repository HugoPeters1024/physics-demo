...
static const char* lighting_skybox_fs_src = R"(
      ...
      void main()
      {
          ...
          float ambientLeft = max(1 - lightingValues.y - lightingValues.z,0);

          color = clamp(vec4(reflectedComponent + refractedComponent + fragColor * lightness * ambientLeft, 1), 0, 1);
      }
    )";
